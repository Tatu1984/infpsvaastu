...
export async function GET(request: NextRequest) {
  try {
    ...
    const propertyCategory = searchParams.get("category") || searchParams.get("propertyCategory")
    ...
    // Filter by property category (RESIDENTIAL or COMMERCIAL)
    // Note: This filter only works after running: npx prisma migrate dev
    // Uncomment the line below once the migration is applied
    // if (propertyCategory && ['RESIDENTIAL', 'COMMERCIAL'].includes(propertyCategory)) {
    ...
    if (propertyType && propertyType !== "All Types") {
      const validTypes = ['APARTMENT', 'HOUSE', 'VILLA', 'PLOT', 'COMMERCIAL', 'PG', 'ROOMMATE',
        'OFFICE_SPACE', 'SHOP', 'SHOWROOM', 'WAREHOUSE', 'INDUSTRIAL', 'COWORKING_SPACE', 'RESTAURANT_SPACE',
        'PENTHOUSE', 'STUDIO', 'INDEPENDENT_FLOOR']
      const upperType = propertyType.toUpperCase().replace(/\s+/g, '_')
  ...
  try {
    ...
    const property = await prisma.property.create({
      data: {
        ...
        // propertyCategory: data.propertyCategory || 'RESIDENTIAL', // Uncomment after migration
